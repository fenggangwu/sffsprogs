...
_("size %s specified for log subvolume is too large, maximum is %lld blocks\n"),
	...
	sbp->sb_razstartblkno = 5;
	sbp->sb_razblkcnt = 5;
	sbp->sb_estartblkno = 1;
	sbp->sb_eblkcnt = 1;
	sbp->sb_istartblkno = 2;
	sbp->sb_iblkcnt = 2;
	sbp->sb_bandsize = 3;
	sbp->sb_superbandsize = 3;
	sbp->sb_ehead = 4;
	sbp->sb_etail = 4;
	...
	if (!qflag || Nflag) {
		...
		printf(_(
		   ...
		   "spsize   =%-22lu razstart=%-3lu razblkcnt=%lu\n"
		   "         =%-22s estart=%-5lu eblkcnt=%lu\n"
		   "         =%-22s istart=%-5lu iblkcnt=%lu\n"
		   "         =%-22s bandsize=%-3u superbandsize=%u\n"
		   "         =%-22s ehead=%-6lu etail=%lu\n"),
			...
			sizeof(xfs_sb_t), sbp->sb_razstartblkno, sbp->sb_razstartblkno,
			"", sbp->sb_estartblkno, sbp->sb_eblkcnt,
			"", sbp->sb_istartblkno, sbp->sb_iblkcnt,
			"", sbp->sb_bandsize, sbp->sb_superbandsize,
			"", sbp->sb_ehead, sbp->sb_etail);
#else
		printf(_(
				   "meta-data=%-22s isize=%-6d agcount=%lld, agsize=%lld blks\n"
				   "         =%-22s sectsz=%-5u attr=%u, projid32bit=%u\n"
...
#ifdef SMR_DBG
	xfs_dsb_t *dsbp = (xfs_dsb_t *)((buf)->b_addr);
	printf("[ sb]razstartbno:%llu\n",(unsigned long long)sbp->sb_razstartblkno);
	printf("[dsb]razstartbno:%llu\n",(unsigned long long)dsbp->sb_razstartblkno);
#endif
	libxfs_writebuf(buf, LIBXFS_EXIT_ON_FAILURE);
	libxfs_purgebuf(buf);
